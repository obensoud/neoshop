		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+	
				'<td style="margin-left:20px;">'+
			  		'<input type="text" class="form-control" id="barcodeScanner'+count+'" placeholder="Barcode scanner" onblur="getProductDataCodeBar('+count+')">'+
			  	'</td>'+		
			  	'<td style="margin-left:20px;">'+
	...
	if(row) {
		...
		if(productId == "") {
			...
			$.ajax({
				...
				type: 'post',
				data: {barCodeScannerVal : productId},
				dataType: 'json',
				success:function(response) {
...
function subAmount() {
	...
	// vat
	var vat = (Number($("#subTotal").val())/100) * 0; //0% TVA
	vat = vat.toFixed(2);
	$("#vat").val(vat);
	$("#vatValue").val(vat);
